...
import {useCreateUser, useDeleteUser, useUpdateUser, useUsers} from "../../api/users-api.ts";
...
const UserList = () => {
    ...
    const {data: roles = [], isLoading: isRoleLoading} = useRoles({refetchOnWindowFocus: false})
                            ...
                            {users
                                ...
                                .map((user) => (
                                    <TableRow key={user.id}>
                                        ...
                                        <TableCell>
                                            <GButton size="small" color="primary" startIcon={<EditIcon/>}
                                                     onClick={() => {
                                                         setUpdatedUser(user)
                                                         setDialogOpen(true);
                                                     }} label="Edit"/>
                                            <GButton size="small" color="primary" startIcon={<DeleteIcon/>}
                                                     disabled={user.roles?.some((role) => role.name.toLowerCase() == "admin")}
                                                     onClick={() => {
                                                         deleteUserApi(user.id);
                                                     }} label="Delete" loading={isDeleting}/>
                                        </TableCell>
                                    </TableRow>
                                ))}
                        </TableBody>
